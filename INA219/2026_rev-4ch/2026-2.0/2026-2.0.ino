...
// Only CH2, CH3 and CH4 short-circuit MOSFETs are connected for this test.
const uint8_t SHORT_GATE_PINS[3]        = {4, 3, 2}; // CH2, CH3, CH4
const uint8_t SHORT_GATE_PIN_COUNT      = 3;
...
{
  if (enabled) {
    for (uint8_t i = 0; i < SHORT_GATE_PIN_COUNT; i++) digitalWrite(SHORT_GATE_PINS[i], LOW);
    delay(MOSFET_DEAD_TIME_MS);
    for (uint8_t i = 0; i < SHORT_GATE_PIN_COUNT; i++) digitalWrite(SHORT_GATE_PINS[i], HIGH);
  } else {
    for (uint8_t i = 0; i < SHORT_GATE_PIN_COUNT; i++) digitalWrite(SHORT_GATE_PINS[i], LOW);
    delay(MOSFET_DEAD_TIME_MS);
  }

  Serial.println(enabled ? F("CH2/CH3/CH4 short circuits ON")
                         : F("CH2/CH3/CH4 short circuits OFF"));
  Serial.flush();
}
...
void setup(){
  ...
  Serial.flush();
  for (uint8_t i = 0; i < SHORT_GATE_PIN_COUNT; i++) {
    digitalWrite(SHORT_GATE_PINS[i], LOW);
    pinMode(SHORT_GATE_PINS[i], OUTPUT);
  }
  Serial.println(F("Only CH2/CH3/CH4 short MOSFETs configured on D4/D3/D2"));
